...
function handleFormSubmit(form, input) {
  ...
  if (!errors) {
    ...
    const element = document.querySelector(`[name='${attributeName}']`).closest('.content-container');
...
const addSlider = async (data) => {
  try {
    ...
    const url = `${BASE_URL}/api/v1/sliders`;
    ...
    if (err.response && err.response.data) {
      ...
      console.log('err', err);
      showAlert('error', 'An error occurred while adding the slider.');
    }
  }
};
...
//     try {
//       const url = `http://127.0.0.1:3000/api/v1/sliders`;
...
const imageOriginalInput = document.querySelector('#imageOriginal');
const imageBoxSecond = document.querySelector('.image-box--2');
const imageLargeInput = document.querySelector('#imageLarge');
const imageBoxThird = document.querySelector('.image-box--3');
const imageMediumInput = document.querySelector('#imageMedium');
const imageBoxFourth = document.querySelector('.image-box--4');
const imageSmallInput = document.querySelector('#imageSmall');

imageBoxFirst.addEventListener('click', () => {
  imageOriginalInput.click();
});
imageUpload(imageOriginalInput, imageBoxFirst, 1);

imageBoxSecond.addEventListener('click', () => {
  imageLargeInput.click();
});
imageUpload(imageLargeInput, imageBoxSecond, 2);

imageBoxThird.addEventListener('click', () => {
  imageMediumInput.click();
});
imageUpload(imageMediumInput, imageBoxThird, 3);

imageBoxFourth.addEventListener('click', () => {
  imageSmallInput.click();
});
imageUpload(imageSmallInput, imageBoxFourth, 4);
