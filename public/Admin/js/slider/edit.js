...
const updateSlider = async (data, id) => {
  try {
    ...
    const url = `${BASE_URL}/api/v1/sliders/${id}`;
    ...
    if (err.response && err.response.data) {
      ...
      console.log('err', err);
      showAlert('error', 'An error occurred while updating the slider.');
    }
  }
};
        ...
        if (result.isConfirmed) {
          try {
            ...
            const res = await axios.patch(
              `${BASE_URL}/api/v1/sliders/${id}/image`,
              data,
              {
                headers: { 'Content-Type': 'multipart/form-data' },
              },
